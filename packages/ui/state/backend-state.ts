...
interface IBackendState {
    ...
    onRunUpdate(runId: string, callback: (run: IRun) => IRunUpdateEvent[]): () => void

    undoBoard(boardId: string): Promise<void>
    redoBoard(boardId: string): Promise<void>

    updateBoardMeta(boardId: string, name: string, description: string, logLevel: ILogLevel, stage: IExecutionStage): Promise<void>
    ...
    // Board Operations
    upsertComment(boardId: string, comment: IComment, append: boolean): Promise<void>
    updateNode(boardId: string, node: INode, append: boolean): Promise<void>
    moveNode(boardId: string, nodeId: string, coordinates: [number, number, number], append: boolean): Promise<void>,
    addNodeToBoard(boardId: string, node: INode, append: boolean): Promise<INode>
    connectPins(boardId: string, fromNode: string, fromPin: string, toNode: string, toPin: string, append: boolean): Promise<void>
    disconnectPins(boardId: string, fromNode: string, fromPin: string, toNode: string, toPin: string, append: boolean): Promise<void>
    removeNode(boardId: string, nodeId: INode, append: boolean): Promise<void>
    removeComment(boardId: string, commentId: IComment, append: boolean): Promise<void>
