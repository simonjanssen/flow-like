...
const useRunExecutionStore = create<IRunExecutionState>((set, get) => ({
    ...
    addRun: async (runId: string, boardId: string, eventIds: string[]) => {
        ...
        const unlisten = await listen(`run:${runId}`, (event: Event<IRunUpdateEvent[]>) => {
            ...
            for (const payload of event.payload) {
                if (payload.method === "add") {
                    if(add_nodes.has(payload.run_id)) {
                        add_nodes.set(payload.run_id, [...add_nodes.get(payload.run_id), ...payload.node_ids])
                        continue;
                    }
                ...
                if(remove_nodes.has(payload.run_id)) {
                    remove_nodes.set(payload.run_id, [...remove_nodes.get(payload.run_id), ...payload.node_ids])
                    continue;
                }
    ...
    removeRun: (runId: string) => set((state) => {
        const runs = new Map(state.runs);
    ...
    addNodesOnRun: (runId: string, nodeIds: string[]) => set((state) => {
    ...
    removeNodesOnRun: (runId: string, nodeIds: string[]) => set((state) => {
        const runs = new Map(state.runs);
        const run = runs.get(runId);
        if (!run) {
            return state;
