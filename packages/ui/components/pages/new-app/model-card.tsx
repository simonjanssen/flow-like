"use client"
import { Check, FileSearch, Package2, PackageCheck, Plus, ScanEye } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage, Badge, Card, CardContent, humanFileSize, IBitTypes, useBackend, useInvoke } from "../../..";
...
export function ModelCard({
    ...
    const bitData = useInvoke(backend.bitState.getBit, backend.bitState, [bitId, hub]);
    ...
    if (
        searchQuery &&
        !bitData.data.meta?.en?.name.toLowerCase().includes(searchQuery.toLowerCase())
    ) {
        return null;
    }
    ...
    if (typeFilter !== "all") {
        if (typeFilter === "embedding" && bitData.data.type !== IBitTypes.Embedding)
            return null;
        if (typeFilter === "image" && bitData.data.type !== IBitTypes.ImageEmbedding)
            return null;
    }
