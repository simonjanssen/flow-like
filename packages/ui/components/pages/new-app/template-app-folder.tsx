...
import { Avatar, AvatarFallback, AvatarImage, Badge, Card, CardContent, useBackend, useInvoke } from "../../..";
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in-0 zoom-in-95 duration-300">
        {templates.map(([templateId, metadata]) => (
            <TemplateCard
                ...
                metadata={metadata}
                selected={selectedTemplate[0] === appId && selectedTemplate[1] === templateId}
...
export function AppTemplateFolder({
    ...
    const appMeta = useInvoke(backend.appState.getAppMeta, backend.appState, [appId]);

    const hasSelectedTemplate = useMemo(() =>
        templates.some(([templateId]) =>
