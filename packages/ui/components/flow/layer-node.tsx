...
import { IValueType, IVariableType, type INode, type IPin, type IPinOptions } from "../../lib";
import { type ILayer, IPinType } from "../../lib/schema/flow/board";
import { CommentDialog } from "./comment-dialog";
import { FlowPin } from "./flow-pin";
import { NameDialog } from "./name-dialog";
import { Button, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Input, Label, ScrollArea, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Separator, Tabs, TabsContent, TabsList, TabsTrigger } from "../ui";
...
const useGroupedPins = (edits: Record<string, PinEdit>) => {
    return useMemo(() => {
        ...
        const inputs = sortByIndex(all.filter((p) => p.pin_type === IPinType.Input));
        const outputs = sortByIndex(all.filter((p) => p.pin_type === IPinType.Output));
...
const LayerEditMenu: React.FC<LayerEditMenuProps> = ({
    ...
    const [edits, setEdits] = useState<Record<string, PinEdit>>(() => buildInitialEdits(layer));
    ...
    const editPin = useCallback((id: string, patch: Partial<PinEdit>) => {
        setPin(id, (p) => {
    ...
    const setIndex = useCallback(
        (id: string, newIndex: number) => {
            ...
            const reordered = [...without.slice(0, clamped), pin, ...without.slice(clamped)];
            reindexGroupInState(reordered);
        },
        [edits, inputs, outputs, reindexGroupInState],
    );

    const uniqueId = useCallback((base: string) => {
        let n = 1;
        let id = `${base}_${n}`;
    ...
    const removePin = useCallback((id: string) => {
        setEdits((prev) => {
            ...
            const remaining = Object.values(copy).filter(p => p.pin_type === pin.pin_type);
    ...
    const applyChanges = useCallback(async () => {
        ...
        const nextInputs = reindex(sortByIndex(Object.values(nextPins).filter(p => p.pin_type === IPinType.Input)));
        const nextOutputs = reindex(sortByIndex(Object.values(nextPins).filter(p => p.pin_type === IPinType.Output)));

        for (const p of nextInputs) nextPins[p.id] = { ...(nextPins[p.id] as IPin), index: p.index };
        for (const p of nextOutputs) nextPins[p.id] = { ...(nextPins[p.id] as IPin), index: p.index };
                ...
                <Tabs value={tab} onValueChange={(v) => setTab(v as any)} className="mt-2">
                    ...
                    <TabsContent value="inputs" className="mt-3 space-y-2">
                        <div className="flex justify-end">
                            <Button size="sm" onClick={() => addPin(IPinType.Input)} className="gap-2">
                    ...
                    <TabsContent value="outputs" className="mt-3 space-y-2">
                        <div className="flex justify-end">
                            <Button size="sm" onClick={() => addPin(IPinType.Output)} className="gap-2">
                                <PlusIcon className="h-4 w-4" />
                                Add Output Pin
                            </Button>
                        </div>
                    ...
                    >
                        <div className="flex items-center gap-2 px-2 py-1.5 border-b">
                            ...
                            <div className="flex-1 truncate text-sm font-medium">
                                ...
                                <span className="ml-2 text-xs text-muted-foreground">({pin.id})</span>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-3">
                            <div className="space-y-1.5">
                                ...
                                <Select
                                    ...
                                    onValueChange={(val) => onEdit(pin.id, { pin_type: val as IPinType })}
                                >
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select
                                    ...
                                    onValueChange={(val) => onEdit(pin.id, { value_type: val as IValueType })}
                                >
                                    <SelectTrigger className="h-8">
                                        <SelectValue />
                                    </SelectTrigger>
                            ...
                            <div className="space-y-1.5 md:col-span-1">
                                ...
                                <Input
                                    className="h-8"
                                    value={pin.schema ?? ""}
                                    onChange={(e) => onEdit(pin.id, { schema: e.target.value || null })}
                                />
                            </div>
                            ...
                            <div className="space-y-1.5 md:col-span-1">
                                ...
                                <Select
                                    value={pin.data_type}
                                    onValueChange={(val) => onEdit(pin.id, { data_type: val as IVariableType })}
                                >
                                    <SelectTrigger className="h-8">
                                        <SelectValue />
                                    </SelectTrigger>
...
const PinOptionsButton: React.FC<PinOptionsButtonProps> = ({ pin, onApply }) => {
                ...
                <DialogContent className="sm:max-w-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                        <div className="flex items-center gap-2 md:col-span-3">
                            ...
                            />
                            <Label htmlFor={`opt-egvt-${pin.id}`} className="text-xs">Enforce Generic VT</Label>
                        </div>
                        ...
                        <div className="flex items-center gap-2 md:col-span-3">
                            ...
                            />
                            <Label htmlFor={`opt-es-${pin.id}`} className="text-xs">Enforce Schema</Label>
                        </div>
                        ...
                        <div className="flex items-center gap-2 md:col-span-3">
                            ...
                            />
                            <Label htmlFor={`opt-sens-${pin.id}`} className="text-xs">Sensitive</Label>
                        </div>
                        ...
                        <div className="space-y-1.5 md:col-span-3">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    setLocal({
                                        ...(local ?? {}),
                                        range: nextRange.length === 2 ? nextRange : nextRange.length === 1 ? [nextRange[0]] : null,
                                    } as IPinOptions);
                                }}
                            />
                        </div>
                        ...
                        <div className="space-y-1.5 md:col-span-6">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setLocal({
                                        ...(local ?? {}),
                                        valid_values: e.target.value.trim() === "" ? null : fromCSVStrings(e.target.value),
                                    } as IPinOptions)
                                }
                            />
                        </div>
