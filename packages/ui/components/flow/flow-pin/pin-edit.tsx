...
export function PinEdit({ pin, defaultValue, changeDefaultValue }: Readonly<{ pin: IPin, defaultValue: any, changeDefaultValue: (value: any) => void }>) {
    ...
    if (pin.data_type === IVariableType.Boolean) return <BooleanVariable pin={pin} value={defaultValue} setValue={changeDefaultValue} />
    if ((pin.options?.valid_values?.length ?? 0) > 0 && pin.data_type === IVariableType.String) return <EnumVariable pin={pin} value={defaultValue} setValue={changeDefaultValue} />

    return <WithMenu pin={pin} defaultValue={defaultValue} changeDefaultValue={changeDefaultValue} />
}


function WithMenuInner({ pin, defaultValue, changeDefaultValue }: Readonly<{ pin: IPin, defaultValue: number[] | undefined | null, changeDefaultValue: (value: any) => void }>) {
