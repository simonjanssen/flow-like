...
import { updateNodeCommand, type INode } from "../../../lib";
...
export function PinEditModal({ appId, boardId }: Readonly<{ appId: string, boardId: string }>) {
    ...
    const onChangeDefaultValue = useCallback(async () => {
        ...
        const command = updateNodeCommand({
            node: {
                ...
                pins: {
                    ...node.pins,
                    [editedPin.pin.id]: { ...editedPin.pin, default_value: defaultValueState },
                },
            },
        });
    ...
    }, [appId, currentNode, editedPin?.node, editedPin?.pin, defaultValueState, refetchBoard, pushCommand, backend, boardId, stopEditPin]);
