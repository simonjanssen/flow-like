...
const FlowNodeInner = memo(({props, onHover} : {
  ...
  const parsePins = useCallback((pins: IPin[]) => {
...
function FlowNode(props: NodeProps<FlowNode>) {
  ...
  if(isOpen || isHovered) {
    return <ContextMenu onOpenChange={(open) => {
      ...
      <ContextMenuTrigger>
        <FlowNodeInner props={props} onHover={setIsHovered} setCommentMenu={setCommentMenu} setRenameMenu={setRenameMenu} />
      </ContextMenuTrigger>
  ...
  return <>
    {commentMenu && <FlowNodeCommentMenu boardId={props.data.boardId} node={props.data.node} open={commentMenu} onOpenChange={(open) => setCommentMenu(open)} />}
    {renameMenu && <FlowNodeRenameMenu boardId={props.data.boardId} node={props.data.node} open={renameMenu} onOpenChange={(open) => setRenameMenu(open)} />}
    <FlowNodeInner props={props} onHover={(hover) => setIsHovered(hover)} setCommentMenu={setCommentMenu} setRenameMenu={setRenameMenu} />
  </>
}
