...
export function CommentNode(props: NodeProps<CommentNode>) {
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger>
        <div key={props.id + "__node"} className={`bg-card p-1 react-flow__node-default selectable !w-[300px] focus:ring-2 relative rounded-md group opacity-80 ${props.selected && "!border-primary border-2"}`}>
          <Dialog open={edit.open} onOpenChange={(open) => {setEdit(old => ({ ...old, open }))}}>
            <DialogContent>
              <DialogHeader>
              ...
              <Textarea rows={6} value={edit.content} onChange={(e) => {
                setEdit({ ...edit, content: e.target.value })
              }} />
              <Button disabled={props.data.comment.content === edit.content} onClick={async () => {
                await props.data.onUpsert({ ...props.data.comment, content: edit.content })
      ...
      <ContextMenuContent className='max-w-20'>
        <ContextMenuItem className='flex flex-row items-center gap-2' onClick={() => {
