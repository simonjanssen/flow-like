...
export function BoardMeta({ appId, boardId, board, version, closeMeta, selectVersion }: {
    ...
    const createVersion = useCallback(async (type: IVersionType) => {
                    ...
                    <Select
                        ...
                        onValueChange={(e) => {
                            if (e === "Latest") {
                                ...
                                const version = e.split('.').map(Number) as [number, number, number];
                    ...
                    >
                        ...
                        <SelectContent>
                            ...
                            {versions.data?.sort((a,b) => {
                                ...
                                return b[2] - a[2];
                            }).map(version => <SelectItem key={version.join('.')} value={version.join('.')}>{version.join('.')}</SelectItem>)}
                        </SelectContent>
                    </Select>
                </div>
