	...
	},
	{
		...
		cell: ({ row }) => {
			...
			return (
				<HoverCard>
					<HoverCardTrigger>
						<p className="max-w-20 overflow-ellipsis overflow-hidden line-clamp-1">
							{path}
						</p>
					</HoverCardTrigger>
					<HoverCardContent>
...
export function FileList({
	...
	onFileDelete?: (file: IFileMetadata) => void;
}>) {
	const columnsDeleteFile: ColumnDef<IFileMetadata>[] = [
		...columns,
		{
			accessorKey: "delete",
			header: () => {
	...
	return (
		<div className="w-full mb-5 mt-2">
			<DataTable
				columns={
					typeof onFileDelete === "function" ? columnsDeleteFile : columns
				}
				data={files}
			>
				{children}
