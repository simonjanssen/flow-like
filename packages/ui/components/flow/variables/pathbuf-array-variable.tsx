...
export function FolderArrayVariable({
	...
	async function loadFiles() {
		...
		const files = (
			await Promise.all(
				folders?.map((folder) =>
					invoke<IFileMetadata[]>("get_folder_meta", { folderPath: folder }),
				),
			)
		).flat();
		setFiles(files);
		...
		<div key={name} className="w-full items-center gap-1.5">
			<div className="flex flex-row items-center gap-2">
				...
				<HoverCard>
					...
					<HoverCardContent>{variable.description}</HoverCardContent>
				</HoverCard>
			</div>
					...
					<div className="flex flex-row items-center gap-2">
						<Button
							...
							onClick={async () => {
								...
								if (!folders) return;
								setFolders((old) =>
									Array.from(new Set([...(old || []), ...(folders || [])])),
								);
							}}
						>
