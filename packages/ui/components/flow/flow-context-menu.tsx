...
import { doPinsMatch, type IBoard } from "../../lib";
		...
		const normalNodes = nodes
			...
			if (board && callRefNode) {
				Object.values(board.nodes).forEach((node) => {
					...
					const pins = Object.values(callRefNode?.pins ?? {}).map(pin => pin.name === "fn_ref" ? {...pin, default_value: convertJsonToUint8Array(node.id)} : pin);
			...
			if (board && variableGetNode && variableSetNode) {
				Object.values(board.variables).forEach((variable) => {
					const getPins = Object.values(variableGetNode?.pins ?? {}).map(pin => pin.name === "var_ref" ? {...pin, default_value: convertJsonToUint8Array(variable.id)} : pin);
					const setPins = Object.values(variableSetNode?.pins ?? {}).map(pin => pin.name === "var_ref" ? {...pin, default_value: convertJsonToUint8Array(variable.id)} : pin);
					const newGetPins = Object.fromEntries(getPins.map(pin => [pin.id, pin]));
					const newSetPins = Object.fromEntries(setPins.map(pin => [pin.id, pin]));
