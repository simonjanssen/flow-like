...
import { ArrowBigLeftDashIcon, NotebookPenIcon, PlayCircleIcon, Redo2Icon, ScrollIcon, Undo2Icon, VariableIcon, XIcon } from 'lucide-react';
...
import { Variable, VariablesMenu } from '../../components/flow/variables/variables-menu';
import {
    ResizableHandle,
    ResizablePanel,
    ResizablePanelGroup,
...
export function FlowBoard({ boardId }: Readonly<{ boardId: string }>) {
    ...
    const board: UseQueryResult<IBoard> = useInvoke("get_board", { boardId: boardId }, [boardId], boardId !== "")
    const openBoards = useInvoke<[string, string][]>("get_open_boards", {})
    const currentProfile: UseQueryResult<ISettingsProfile | undefined> = useInvoke("get_current_profile", {})
    const { addRun, removeRun } = useRunExecutionStore()

    const [traces, setTraces] = useState<{ node?: INode, traces: ITrace[] } | undefined>(undefined)
    ...
    const [pinCache, setPinCache] = useState<Map<string, [IPin, INode]>>(new Map())
    ...
    const placeNode = useCallback(async (node: INode, position?: { x: number, y: number }) => {
        const location = screenToFlowPosition({ x: position?.x ?? clickPosition.x, y: position?.y ?? clickPosition.y });
        const new_node: INode = await executeCommand("add_node_to_board", { boardId: boardId, node: { ...node, coordinates: [location.x, location.y, 0] } })
        if (droppedPin) {
            const pinType = droppedPin.pin_type === "Input" ? "Output" : "Input"
            const pinDataType = droppedPin.data_type

            const pin = Object.values(new_node.pins).find(pin => pin.pin_type === pinType && pin.data_type === pinDataType)
            const [sourcePin, sourceNode] = pinCache.get(droppedPin.id) || []
            if (!sourcePin || !sourceNode) return

            await executeCommand("connect_pins", {
                boardId: boardId,
                fromNode: droppedPin.pin_type === "Output" ? sourceNode.id : new_node.id,
    ...
    const handleDrop = useCallback(async (event: any) => {
        ...
        const getVarNode = catalog.data?.find(node => node.name === `variable_${operation}`)
        if (!getVarNode) return console.dir(catalog.data)

        const varRefPin = Object.values(getVarNode.pins).find(pin => pin.name === "var_ref")
        if (!varRefPin) return
    ...
    useEffect(() => {
        ...
        const { nodes, edges, cache } = parseBoard(board.data, executeBoard, openTraces, executeCommand, selected.current, currentRun, currentProfile.data?.flow_settings.connection_mode)
    ...
    const miniSearch = useMemo(() => new MiniSearch({
        fields: ['name', 'friendly_name', "category", "description", "pin_in_names", "pin_out_names"],
        storeFields: ['id'],
    }), [])
    ...
    useEffect(() => {
        ...
        miniSearch.addAll(catalog.data.map(node => ({ ...node, "pin_in_names": Object.values(node.pins).filter(pin => pin.pin_type === "Input").map(pin => pin.name).join(" "), "pin_out_names": Object.values(node.pins).filter(pin => pin.pin_type === "Output").map(pin => pin.name).join(" ") })));
    ...
    const onNodesChangeIntercept: OnNodesChange = useCallback(
        (changes: any[]) => setNodes((nds) => {
            ...
            const selectChanges = changes.filter((change: any) => change.type === "select")
            ...
            executeCommands(removeChanges.map(change => {
                let foundNode = Object.values(board.data?.nodes || {}).find(node => node.id === change.id)
                if (foundNode) return { command: "remove_node_from_board", args: { boardId: boardId, node: foundNode } }
                let foundComment = Object.values(board.data?.comments || {}).find(comment => comment.id === change.id)
                if (foundComment) return { command: "remove_comment", args: { boardId: boardId, comment: foundComment } }
                return undefined
    ...
    const onEdgesChange: OnEdgesChange = useCallback(
        (changes: any[]) => setEdges((eds) => {
            if (!changes || changes.length === 0) return applyEdgeChanges(changes, eds)
            ...
            for (const change of selectChanges) {
                ...
                if (selectedEdge.data_type !== "Execution") eds = eds.map(edge => edge.id === selectedId ? { ...edge, animated: !change.selected } : edge)
            }

            const removeChanges = changes.filter((change: any) => change.type === "remove")
            executeCommands(removeChanges.map((change: any) => {
                const selectedId = change.id
                ...
                return { command: "disconnect_pins", args: { boardId: boardId, fromNode: fromNode.id, fromPin: fromPin.id, toNode: toNode.id, toPin: toPin.id } }
    ...
    const onReconnect = useCallback(async (oldEdge: any, newConnection: Connection) => {
        ...
        await executeCommand("disconnect_pins", { boardId, fromNode: oldEdge.source, fromPin: oldEdge.sourceHandle, toNode: oldEdge.target, toPin: oldEdge.targetHandle })
        await executeCommand("connect_pins", { boardId, fromNode: newConnection.source, fromPin: newConnection.sourceHandle, toNode: newConnection.target, toPin: newConnection.targetHandle }, true)

        edgeReconnectSuccessful.current = true;
        setEdges((els) => reconnectEdge(oldEdge, newConnection, els));
    }, [boardId, setEdges]);

    const onReconnectEnd = useCallback(async (event: any, edge: any) => {
        if (!edgeReconnectSuccessful.current) {
            const { source, target, sourceHandle, targetHandle } = edge;
            await executeCommand("disconnect_pins", { boardId, fromNode: source, fromPin: sourceHandle, toNode: target, toPin: targetHandle })
            ...
            <div className="flex items-center justify-center absolute translate-x-[-50%] left-[50dvw] top-5 z-40">
                ...
                <FlowDock items={[
                    ...
                    ...(currentRun ? [{
                        ...
                        title: "Logs",
                        onClick: async () => {
                            setTraces(old => old ? undefined : { node: undefined, traces: currentRun.traces })
                        }
                    }] : [] as any),
                ]} />
            </div>
            <ResizablePanelGroup direction="horizontal">
                <ResizablePanel autoSave='flow-variables' defaultSize={0} collapsible={true} collapsedSize={0} ref={varPanelRef}>
                    {board.data && <VariablesMenu board={board.data} executeCommand={executeCommand} />}
                </ResizablePanel>
                ...
                <ResizablePanel autoSave='flow-main-container'>
                    <ResizablePanelGroup direction='vertical'>
                        <ResizablePanel autoSave='flow-main' ref={flowPanelRef}>
                            <FlowContextMenu
                                ...
                                onCommentPlace={async () => {
                                    const location = screenToFlowPosition({ x: clickPosition.x, y: clickPosition.y });
                                ...
                                onFilterSearch={(filter) => {
                                    ...
                                    const search = miniSearch.search(filter, {
                                        prefix: true,
                                        fuzzy: 0.2,
                                        boost: { name: 1, friendly_name: 2, category: 0.5, description: 0.5, pin_in_names: 0.5, pin_out_names: 0.5 }
                                    })

                                    const ids: Set<string> = new Set(search.map((node: any) => node.id))
                                    setFilteredNodes(catalog.data?.filter(node => ids.has(node.id)) || [])
                                }}
                            >
                                <div className={`w-full h-full relative ${isOver && "border-green-400 border-2 z-10"}`} ref={setNodeRef}>
                        ...
                        <ResizableHandle withHandle />
                        <ResizablePanel hidden={!traces || !currentRun} ref={logPanelRef} collapsible={true} autoSave='flow-log' >
                            {traces && currentRun && <Traces node={traces.node} result={currentRun} traces={traces.traces} onOpenChange={(open) => {
