...
export function FlowNodeRenameMenu({ node, boardId, open, onOpenChange }: Readonly<{ node: INode, boardId: string, open: boolean, onOpenChange: (open: boolean) => void }>) {
  const queryClient = useQueryClient()
  const [friendlyName, setFriendlyName] = useState(node.friendly_name)

  async function saveComment() {
    await invoke("update_node", { boardId: boardId, node: { ...node, friendly_name: friendlyName} })
    onOpenChange(false)
    setFriendlyName("")
    refetchBoard()
  }
