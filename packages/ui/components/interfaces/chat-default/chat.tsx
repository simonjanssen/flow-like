...
const ChatInner = (
    ...
    return (
        <main className="flex flex-col h-full w-full items-center grow bg-background max-h-full overflow-hidden ">
            <div className="h-full grow flex flex-col bg-background max-h-full w-full overflow-auto">
                {/* Messages Container */}
                <div
                    ref={scrollContainerRef}
                    onScroll={handleScroll}
                    className="flex-1 overflow-y-auto p-4 space-y-8  flex flex-col items-center grow max-h-full overflow-hidden"
                >
                    {localMessages.map((message) => (
                        <div className="w-full max-w-(--breakpoint-lg) px-4" key={message.id}>
                            <MessageComponent
                                message={message}
                    ...
                    {currentMessage && (
                        <div
                            className="w-full max-w-(--breakpoint-lg) px-4 relative"
                            key={currentMessage.id}
                        >
                            <PuffLoader
                                color={resolvedTheme === "dark" ? "white" : "black"}
                                className="mt-2 absolute left-0 top-0 translate-y-10 -translate-x-full"
                                size={30}
                            />
                ...
                {/* ChatBox */}
                <div className="bg-transparent pb-4 max-w-(--breakpoint-lg) w-full mx-auto">
                    {defaultActiveTools && (
                        <ChatBox
                            ref={chatBox}
