...
const ContainerInner = forwardRef<ISidebarActions, IContainerProps>(
	({ children }, ref) => {
		...
		return (
			<div className="h-full overflow-hidden flex flex-col flex-grow max-h-full">
				...
				{isMobile && (
					<>
						<Sheet open={isOpen} onOpenChange={setIsOpen}>
							<SheetContent
								side="left"
								className="w-80 p-0 border-r bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60"
							>
								{sidebar}
							</SheetContent>
						</Sheet>
						...
						<div className="relative h-full flex flex-col flex-grow max-h-full overflow-hidden">
							{memoizedChildren}
						</div>
					</>
				)}
