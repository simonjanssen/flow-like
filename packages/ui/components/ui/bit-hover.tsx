...
export function BitHover({bit, children}: Readonly<{bit: IBit, children: React.ReactNode}>) {

    const [bitData] = useState<Bit>(bit instanceof Bit ? bit : Bit.fromObject(bit))
    ...
    return (
        <HoverCard>
            ...
            <HoverCardContent >
                ...
                <br />
                <div className='flex flex-row items-center gap-2'>
                    {"languages" in bitData.parameters && bitData.parameters.languages.map((lang: string) => <Badge key={lang}>{lang}</Badge>)}
