...
export class Download {
    ...
    total() {
        const max = Array.from(this.progress.values()).reduce((acc, progress) => acc + progress.max, 0)
        const downloaded = Array.from(this.progress.values()).reduce((acc, progress) => acc + progress.downloaded, 0)
    ...
    measureSpeed() {
        ...
        const lastDownloaded = this.speed.lastPoints.values().reduce((acc, val) => acc + val, 0)
        ...
        return { bytesPerSecond, total: downloaded, progress: downloaded / max * 100, max }
    }
...
export class Bit implements IBit {
    ...
    parameters: IImageEmbeddingModelParameters | IEmbeddingModelParameters | ILlmParameters | IVlmParameters | object = {};
    ...
    public async fetchDependencies(): Promise<BitPack> {
        const cachedDependencies: IBit[] | undefined = await get(this.dependency_tree_hash);
    ...
    async download(cb?: (progress: Download) => void): Promise<IBit[]> {
        try {
            ...
            for (const deps of dependencies.bits) {
                unlistenFn.push(await listen(`download:${deps.hash}`, (event: { payload: IDownloadProgress[] }) => {
            ...
            const download: IBit[] = await invoke("download_bit", { bit: this.toObject() })
