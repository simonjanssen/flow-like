...
export function parseBoard(
    ...
    Object.values(board.comments).forEach(comment => {//#endregion
        nodes.push({
            id: comment.id,
            type: 'commentNode',
            position: { x: comment.coordinates[0], y: comment.coordinates[1] },
            data: { label: comment.id, boardId: board.id, comment: comment, onUpsert: (comment: IComment) => executeCommand("upsert_comment", { boardId: board.id, comment: comment }, false) },
...
export async function handlePaste(
    ...
    try {
        ...
        await executeCommand("paste_nodes_to_board", { boardId: boardId, nodes: nodes, comments: comments, offset: [cursorPosition.x, cursorPosition.y, 0] })
        return
    } catch (error) { }
    ...
    try {
        ...
        await executeCommand("upsert_comment", { boardId: boardId, comment: comment })
        return;
    } catch (error) {
