...
#[tracing::instrument(name = "PUT /bit/{bit_id}/{language}", skip(state, user, meta))]
pub async fn push_meta(
    ...
    if let Some(existing_meta) = &existing_meta {
        ...
        updated_meta.long_description = Set(meta.long_description.or(existing_meta.long_description.clone()));
        ...
        updated_meta.organization_specific_values = Set(
    meta.organization_specific_values.as_ref()
        .and_then(|v| from_slice(v).ok())
        .or(existing_meta.organization_specific_values.clone()),
);
        updated_meta.release_notes = Set(
            meta.release_notes
                .or(existing_meta.release_notes.clone()),
        );
        ...
        updated_meta.use_case = Set(
            meta.use_case
                .or(existing_meta.use_case.clone()),
        );
    ...
    let new_meta = meta::ActiveModel {
        ...
        icon: Set(meta.icon),
        organization_specific_values: Set(
            meta.organization_specific_values
                .as_ref()
                .and_then(|v| from_slice(v).ok()),
        ),
