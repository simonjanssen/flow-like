...
#[tracing::instrument(name = "PUT /bit/{bit_id}", skip(state, user, bit))]
...
async fn download_and_hash(bit: &mut bit::Model, state: AppState) -> flow_like_types::Result<()> {
    ...
    let supports_ranges = response.headers().get(ACCEPT_RANGES)
        .map(|v| v.to_str().unwrap_or("").contains("bytes"))
        .unwrap_or(false);

    let e_tag = response.headers().get(ETAG)
        .and_then(|v| v.to_str().ok())
        .map(|s| s.trim().trim_matches('"').to_string())
        .unwrap_or_else(|| create_id());

    let path =
        flow_like_storage::object_store::path::Path::from("bits").child(e_tag.clone());
            ...
            loop {
                match client.get(url)
                    ...
                    Ok(chunk_response) => {
                        ...
                        pending_upload = Some(flow_like_types::tokio::spawn(async move {
                            upload_fut.await
                        }));

                        total_downloaded += end - start + 1;
                        tracing::info!("Downloaded {}/{} bytes ({:.1}%)",
                    ...
                    Err(e) if retry_count < MAX_RETRIES => {
                        retry_count += 1;
                        tracing::warn!("Retry {}/{} for range {}-{}: {}",
                            retry_count, MAX_RETRIES, start, end, e);
                        flow_like_types::tokio::time::sleep(std::time::Duration::from_secs(2u64.pow(retry_count))).await;
                    },
        ...
        while let Some(chunk_result) = download_stream.next().await {
            match chunk_result {
                Ok(chunk) => {
                    ...
                    if total_downloaded % (100 * 1024 * 1024) == 0 { // Log every 100MB
    ...
    tracing::info!("Successfully processed {} bytes with hash {}", total_downloaded, file_hash);
    Ok(())
}

#[tracing::instrument(name = "build_dependency_hash", skip(bit, state))]
async fn build_dependency_hash(bit: &mut bit::Model, state: AppState) -> flow_like_types::Result<()> {
    ...
    for dependency in dependencies {
        ...
        if hub == state.platform_config.domain {
            let local_bit = bit::Entity::find_by_id(id).one(&state.db).await?
                .ok_or_else(|| flow_like_types::Error::msg(format!("Local bit not found: {}", id)))?;
            hasher.update(local_bit.dependency_tree_hash.as_bytes());
        } else {
            let hub = flow_like::hub::Hub::new(hub, http_client.clone()).await?;
            let remote_bit = hub.get_bit(id).await
                .map_err(|e| flow_like_types::Error::msg(format!("Failed to fetch remote bit {}: {}", id, e)))?;
    ...
    tracing::info!("Built dependency hash for bit {}: {}", bit.id, bit.dependency_tree_hash);
