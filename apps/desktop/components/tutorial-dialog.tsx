...
import { Book, Github, MessageCircle, Star, Sparkles, Code2, Zap, Heart, Rocket, X } from "lucide-react";
...
export function TutorialDialog() {
    ...
    const FloatingIcon = ({ icon: Icon, className = "", delay = 0, duration = 4 }: FloatingIconProps) => (
        <div
    ...
    const AnimatedBackground = ({ children, variant = "welcome" }: AnimatedBackgroundProps) => (
        <div className="relative min-h-screen flex items-center justify-center p-6">
            <div className={`absolute inset-0 ${getBackgroundGradient(variant)}`} />
            <div className="absolute inset-0 bg-gradient-to-tr from-accent/10 via-transparent to-primary/10 animate-pulse"
                 style={{ animationDuration: '8s' }} />
            <div className="absolute inset-0 bg-gradient-to-bl from-secondary/8 via-transparent to-accent/8 animate-pulse"
            ...
            <div className="absolute top-[20%] left-[5%] w-32 h-32 bg-primary/8 rounded-full blur-2xl animate-pulse"
                 style={{ animationDuration: '6s' }} />
            <div className="absolute bottom-[25%] right-[8%] w-24 h-24 bg-secondary/8 rounded-full blur-2xl animate-pulse"
    ...
    const stepData = {
        welcome: {
            ...
            description: "Your comprehensive solution for modern software development"
        },
        docs: {
    ...
    const FeatureItem = ({ icon: Icon, text, color = "primary" }: {
        icon: React.ComponentType<{ className?: string }>;
        text: string;
        color?: string;
    }) => (
    ...
    const BulletPoint = ({ text, color = "primary" }: { text: string; color?: string }) => (
        ...
        <div className="grid grid-cols-2 gap-8 h-full p-8">
            <div className="flex flex-col justify-center">
                <div className="relative mb-6">
                    <img src="/app-logo.webp" alt="Flow Like Logo" className="w-28 h-28 mx-auto" />
                </div>
        ...
        <div className="grid grid-cols-2 gap-8 h-full p-8">
            ...
            <div className="flex flex-col justify-center space-y-6">
                ...
                <Button
                    ...
                    onClick={() => window.open("https://github.com/TM9657/flow-like", "_blank")}
                >
                    ...
                    <div className="p-8 border-b border-border/30 bg-background/15 backdrop-blur-xl">
                        <div className="flex items-center justify-between">
                            <div className="text-center flex-1">
                                <h1 className="text-3xl font-bold">{stepData[currentStep].title}</h1>
                                <p className="text-muted-foreground mt-2 text-lg">{stepData[currentStep].description}</p>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                            ...
                            {STEPS.map((step) => (
                                <div
                                    ...
                                    className={`w-3 h-3 rounded-full transition-all duration-300 ${
                                        step === currentStep ? "bg-primary scale-125 shadow-lg" : "bg-muted-foreground/40"
                                    }`}
                                />
                            ))}
                        </div>
