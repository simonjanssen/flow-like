"use client"
import { IApp, IBackendState, IBit, IBitPack, IBitTypes, IBoard, IDownloadProgress, IExecutionStage, IFileMetadata, IGeneric, ILogLevel, INode, IProfile, IRun, IRunUpdateEvent, ISettingsProfile, useBackendStore, useDownloadManager } from "@tm9657/flow-like-ui";
import { invoke } from "@tauri-apps/api/core";
import { open } from "@tauri-apps/plugin-dialog";
import { type Event, listen, UnlistenFn } from "@tauri-apps/api/event";
...
export class TauriBackend implements IBackendState {
    ...
    async getBoardSettings(): Promise<"straight" | "step" | "simpleBezier"> {
        let profile: ISettingsProfile = await invoke("get_current_profile");
        return profile.flow_settings.connection_mode
    }

    async createRun(appId: string, boardId: string, startIds: string[]): Promise<string> {
    ...
    async executeRun(appId: string, runId: string, cb?: (event: IRunUpdateEvent[]) => void) {
    ...
    async downloadBit(bit: IBit, pack: IBitPack, cb?: (progress: IDownloadProgress[]) => void): Promise<IBit[]> {
...
export function TauriProvider({children}: Readonly<{children: React.ReactNode}>) {
