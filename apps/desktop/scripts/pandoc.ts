...
interface Asset {
  ...
  function extractFileFromZip(zipFilePath: string, fileToExtract: string, outputPath: string) {
  ...
  async function getLatestRelease(): Promise<{ assets: Asset[] }> {
    ...
    const response = process.env.GITHUB_TOKEN ? await axios.get(url, {
      headers: {
        ...
        .pipe(tar.t({ 
          onentry: (entry: any) => {
            ...
            if (fileName === fileToExtract) {
              ...
              console.log(`File '${fileToExtract}' successfully extracted to '${outputPath}'.`);
            }
          }
        }))
        .on('error', (err: any) => console.error(`Error extracting file: ${err}`))
        .on('end', () => {
    ...
    try {
      ...
      for (const asset of latestRelease.assets) {
        if (asset.name.endsWith('arm64-macOS.zip')) {
          ...
          extractFileFromZip("arm-macos.zip", "pandoc", "pandoc-aarch64-apple-darwin");
        ...
        if (asset.name.endsWith('x86_64-macOS.zip')) {
          ...
          extractFileFromZip("amd-macos.zip", "pandoc", "pandoc-x86_64-apple-darwin");
        ...
        if(asset.name.endsWith('windows-x86_64.zip')) {
          ...
          extractFileFromZip("amd-windows.zip", "pandoc.exe", "pandoc-x86_64-pc-windows-msvc.exe");
