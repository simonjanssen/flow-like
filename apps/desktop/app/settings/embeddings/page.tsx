...
export default function Page() {
    ...
    const embeddingModels : UseQueryResult<Bit[]> = useInvoke("get_bits_by_category", {bitType: "Embedding"}, ["Embedding", profile.data?.hub_profile.id ?? ""], typeof profile.data !== "undefined")
    const imageEmbeddingModels : UseQueryResult<Bit[]> = useInvoke("get_bits_by_category", {bitType: "ImageEmbedding"}, ["ImageEmbedding", profile.data?.hub_profile.id ?? ""], typeof profile.data !== "undefined")
    ...
    async function imageBlacklist(){
        ...
        const dependencies = await Promise.all(imageEmbeddingModels.data.map(bit => bit.fetchDependencies()))
        const blacklist = new Set<string>(dependencies.map(dep => dep.bits.filter(bit => bit.type !== "ImageEmbedding").map(bit => bit.id)).flat())
    ...
    return <main className="justify-start flex flex-row items-start w-full pr-4">
        <div className="mr-6 invisible-scroll">
            <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">Embedding Models</h1>
            <br />
            ...
            {(embeddingModels.data && imageEmbeddingModels.data) &&
                <BentoGrid className="mx-auto cursor-pointer w-full pb-20">
                    {(embeddingModels.data && imageEmbeddingModels.data) && [...embeddingModels.data, ...imageEmbeddingModels.data].sort((a,b) => Date.parse(b.updated) - Date.parse(a.updated)).filter(bit => !blacklist.has(bit.id)).map((bit, i) => {
                        if (i === 0) counter = 0;
                        let wide = counter === 3 || counter === 6;
                        if (counter === 6) counter = 0;
                        else counter += 1;
