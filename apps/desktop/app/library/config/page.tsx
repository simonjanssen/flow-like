...
import { IAppCategory, IAppStatus, IAppVisibility } from "@tm9657/flow-like-ui";
...
export default function Id() {
    ...
    useEffect(() => {
        ...
        const metadataChanged = JSON.stringify(metadata.data) !== JSON.stringify(localMetadata);
                            ...
                            <Select
                                ...
                                onValueChange={(value) => {
                                    if (localApp && canEdit) {
                                        setLocalApp({
                                            ...localApp,
                                            secondary_category: value === "none" ? null : value as IAppCategory,
                                        });
                                    }
                                }}
                                disabled={!canEdit}
                                        ...
                                        <SelectItem key={status} value={status}>
                                            <div className="flex items-center gap-2">
                                                <div className={`w-2 h-2 rounded-full ${
                                                    status === IAppStatus.Active ? 'bg-green-500' :
                                                    status === IAppStatus.Inactive ? 'bg-yellow-500' :
                                                    'bg-gray-500'
                                                }`} />
                                                {status}
                                            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 border rounded-lg">
                            <div className="text-2xl font-bold text-blue-600">{localApp.download_count}</div>
                            <div className="text-sm text-gray-600 dark:text-gray-400">Downloads</div>
                        </div>
                        <div className="text-center p-4 border rounded-lg">
                            <div className="text-2xl font-bold text-green-600">{localApp.rating_count}</div>
                            <div className="text-sm text-gray-600 dark:text-gray-400">Ratings</div>
                        </div>
                        <div className="text-center p-4 border rounded-lg">
                            <div className="text-2xl font-bold text-purple-600">{localApp.interactions_count}</div>
                ...
                <Card className="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950">
                    <CardContent className="pt-6">
                        <div className="flex items-center gap-2 text-blue-800 dark:text-blue-200">
                            ...
                            <span className="text-sm">You don't have edit permissions for this application</span>
                        </div>
                    </CardContent>
                </Card>
            )}
