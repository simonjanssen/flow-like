...
export default function ProfileCreation() {
    ...
    const [stats, setStats] = useState<{ time: number, timeString: string, speed: number, total: number, progress: number, max: number }[]>([])
    ...
    async function calculateSize() {
        const sizes = await Promise.all(bits.map(bit => Bit.fromObject(bit).fetchSize()))
    ...
    useEffect(() => {
        ...
        const selected = new Set(searchParams.getAll("profiles").map(profile => profile.replaceAll("profiles=", "")))
        if (selected.size === 0) router.push("/onboarding")
        
        const filteredProfiles = defaultProfiles.data.filter(profile => selected.has(profile[0].hub_profile.id ?? ""))
    ...
    return <div className='p-4 max-w-screen-lg w-full'>
        <div className='flex flex-row items-center'>
            <div className='w-full'>
                <h1>🚀 Great Selection!</h1>
                <h2>Let´s download your models <b className='highlight'>{(stats[stats.length - 1]?.progress || 0).toFixed(2)}%</b> finished 🤩</h2>
        ...
        <div className='flex flex-row items-center justify-end flex-nowrap gap-2'>
                {bits.map(bit => <BitDownload key={bit.id} onDownload={onDownload} bit={bit} />)}
            </div>     
...
function BitDownload({ bit, onDownload }: Readonly<{ bit: Bit, onDownload: (progress: IDownloadProgress) => void }>) {
    useEvent(`download:${bit.hash}`, (event: { payload: IDownloadProgress[] }) => {
