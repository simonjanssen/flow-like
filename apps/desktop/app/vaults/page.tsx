...
import {
	...
	HoverCardTrigger,
	Separator,
} from "@tm9657/flow-like-ui/components/ui";
import {
	AlertTriangle,
	FilesIcon,
	LinkIcon,
				...
				<div className="flex flex-row items-center flex-wrap min-h-full flex-grow h-full gap-4">
					{vaults.data?.length === 0 && (
						<EmptyState
							...
							icons={[VaultIcon, FilesIcon, LinkIcon]}
							className="min-w-full min-h-full flex-grow h-full"
							title="No Vaults Found"
							description="Create a custom vault based on your Data for Free and Secure."
						/>
					)}
					{vaults.data
						?.sort(
...
function Vault({ vault }: Readonly<{ vault: IVault }>) {
	const router = useRouter();
	const vault_size = useInvoke<number>(
		"get_vault_size",
		{ vaultId: vault.id },
		[vault.id],
	);
