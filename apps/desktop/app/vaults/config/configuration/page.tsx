...
export default function Id() {
    ...
    const variables = useMemo(() => {
        const vars = (boards.data ?? []).map(board => {
            return [board, Object.values(board.variables).filter(variable => variable.exposed && variable.editable).sort((a, b) => a.name.localeCompare(b.name))]
        }).filter(([boards, node]) => node.length > 0).sort((a, b) => (a[0] as IBoard).name.localeCompare((b[0] as IBoard).name)) as [IBoard, IVariable[]][]

        return vars
    }, [boards.data])

    async function upsertVariable(board: IBoard, variable: IVariable) {
        await invoke("get_vault_board", { vaultId: id, boardId: board.id, pushToRegistry: true })
    ...
    return <main className="justify-start flex flex-col items-start w-full flex-1 max-h-full overflow-y-auto flex-grow gap-4">
        ...
        <Accordion type="multiple" className="w-full">
            {variables.map(([board, vars]) => <AccordionItem className='rounded-md px-2' value={board.id} key={board.id}>
                ...
                {
                    vars.map(variable => <AccordionContent key={variable.id} className='px-2 grid w-full max-w-sm items-center gap-1.5'>
                        <Label className=''>{variable.name}</Label>
                            <VariablesMenuEdit variable={variable} updateVariable={async (variable) => {
