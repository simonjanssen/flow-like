...
export default function NotificationsPage() {
    ...
    const invitations: IInvite[] = invitationPages
        ? invitationPages.pages.flatMap((page) => page)
        : [];

    const handleInviteAction = useCallback(async (id: string, action: "accept" | "decline") => {
        try {
            if (action === "accept") {
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                    ...
                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                        ...
                                        {getTimeAgo(new Date(Date.parse(invite.created_at)).toLocaleString())}
                                    </div>
                                </div>
                            </div>
                        </div>
