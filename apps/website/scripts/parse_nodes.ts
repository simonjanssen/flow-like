...
const nodeJson = JSON.parse(fs.readFileSync("./src/assets/site.json").toString())
...
for (const [pin, node] of cache.values()) {
    ...
    for (const connectedTo of pin.connected_to) {
        const [conntectedPin, connectedNode] =
...
fs.writeFileSync("./src/assets/board.json", JSON.stringify({
    nodes: parsedNodes,
    edges: parsedEdges,
}))