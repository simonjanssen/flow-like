...
async fn translate_board(path: &str) -> flow_like_types::Result<()> {
    ...
    let file_name = path_buf.file_name().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "File name not found")
    })?;
    let object_store = flow_like_storage::object_store::local::LocalFileSystem::new_with_prefix(parent_string)?;
    let object_store: Arc<dyn flow_like_storage::object_store::ObjectStore> = Arc::new(object_store);
    ...
    let json = flow_like_types::json::to_string_pretty(&board).map_err(|e| {
        std::io::Error::new(std::io::ErrorKind::InvalidData, format!("Failed to serialize board: {}", e))
    })?;
    fs::write(&path_buf, json).map_err(|e| {
        std::io::Error::new(std::io::ErrorKind::WriteZero, format!("Failed to write file: {}", e))
    })?;
