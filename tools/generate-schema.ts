...
const schemaFiles = getAllFiles(schemaDir).filter(file => file.endsWith('.json'));

schemaFiles.forEach(schemaFile => {
    const relativePath = relative(schemaDir, schemaFile); // e.g., "llm/history.json"
    const parsedPath = parse(relativePath);
    const outputFilePath = join(outputDir, parsedPath.dir, `${parsedPath.name}.ts`); // e.g., "lib/schema/llm/history.ts"
    ...
    prefixes.forEach(prefix => {
        ...
        content = content.replaceAll(new RegExp(`\\b${prefix}(?=[ ;\\[])`, 'g'), `I${prefix}`);
    })
